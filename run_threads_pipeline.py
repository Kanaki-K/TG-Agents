...
⚠️ КУДА едет отложка: в THREADS_TEST_CHANNEL из .env, а если он не задан — в БОЕВОЙ PUBLISH_CHANNEL
(тот же, куда публикуется настоящий флагман/скоуп; аудит 15.07 поймал, что докстрока обещала
«тестовый канал», а код молча ехал в боевой). Хочешь обкатку без риска — заведи тестовый канал,
дай админку MTProto-аккаунту и пропиши THREADS_TEST_CHANNEL=@имя в .env.
...
def run_threads_cycle(hint: str = "", publish: bool = True, emit=print) -> str:
    ...
    if not channel:
        ...
        if channel:
            out("⚠️ THREADS_TEST_CHANNEL не задан — еду в БОЕВОЙ канал. Серия ляжет в «Отложенные» "
                "рядом с настоящими постами: проверь и удали её оттуда, НЕ дай выйти в эфир.")
    if not channel:

...
(run_pipeline._pick_timely_theme → published_journal.record), поэтому join
...
# Подслой N.M, вынесенный в СВОЮ категорию (перекрывает макро-слой). Хотим видеть отдельно,
# стреляет ли разряд, а не топить в «применении»:
#  • 1.2 AI×крипта — «уникальное ядро» (держать ≥30%);
#  • 1.5 Институции/фонды/TradFi — запрос аудитории (опрос 25.07: фонды/институции 46%, фонд.рынок
#    23%); раньше был размазан по «применению», теперь мерим и рулим отдельно (задача 29.07).
SUBLAYER_OVERRIDES = {
    "1.2": ("ai-крипта", "AI × крипта (уникальное ядро)"),
    "1.5": ("институции", "Институции, фонды и TradFi"),
}
UNKNOWN = "unknown"

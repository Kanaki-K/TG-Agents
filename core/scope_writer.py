...
POLISH = (
    "ФИНАЛЬНАЯ ШЛИФОВКА ПОДАЧИ (scope 🔭). Ниже — ГОТОВЫЙ ПО ФАКТАМ черновик: гейт повода, цифры и 2FA уже "
    "пройдены. Твоя единственная задача — ПЕРЕПИСАТЬ его в ПОДАЧУ ЭТАЛОНА K3 (scope_manual §8.1-b в контексте): "
    "скелет хук-цифра → факты с именами → разговорная СВЯЗКА-МОСТ («Веду к тому...» / «Ну вот...») → ФИНАЛ = "
    "ОБРАЗ-АФОРИЗМ авторским голосом; простой язык без англицизмов/жаргона; каждый факт-контраст со слоем "
    "«значит...». ⛔ ФАКТЫ, ЦИФРЫ, ИМЕНА, ДАТЫ НЕ МЕНЯЙ — только голос и структуру. ⛔ Финал НЕ хедж («ориентир, "
    "а не сигнал» и пр.). Футер сохрани ДОСЛОВНО со ссылками. Вызови save_draft(kind='scope') с ФИНАЛОМ (линтер "
    "вернёт правки — устрани и сохрани ещё раз), затем выведи ТОЛЬКО финальный пост, без преамбулы и меты.\n\n"
    "ЧЕРНОВИК:\n{draft}"
)

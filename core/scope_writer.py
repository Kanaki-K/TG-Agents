...
TASK = (
    ...
    "Голый пересказ новости = провал (стал новостником). Длина: ⭐ ТЕЛО ≤500 знаков (БЕЗ футера) — ЖЁСТКИЙ "
    "лимит: тот же текст репостится в Threads (лимит 500), формат ДУАЛЬНЫЙ ТГ+Threads. Активно УЖИМАЙ до тела "
    "~350–480 (короткий = быстро вкурсе + твой едж, НЕ разбор); >500 тела = провал (не влезет в Threads). ОДИН "
    ...
    "4) save_draft(kind='scope') — линтер вернёт правки/предупреждения (ТЕЛО >500 = не влезет в Threads, ярлыки-разделы, фурнитура 💡/💭); "
...
FIX = (
    ...
    "длину (ТЕЛО ≤500 знаков без футера — дуал ТГ+Threads; цель ~350–480) и формат scope (без флагман-фурнитуры-ярлыков):\n"

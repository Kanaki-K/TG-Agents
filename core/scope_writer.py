...
# Письмо scope — Sonnet. Пробовали Opus (13.07) ради подачи — прогон вышел ~$1.6 ДАЖЕ без Скаута и без поиска
# обложки: весь агентный цикл (поиск/гейт/верификация/письмо) крутится на одной модели, и Opus платится за
# дёрганье инструментов, а не только за письмо. При ~8-16 scope/мес это раздувало бюджет вдвое. Решение
# владельца 13.07: вернуть Sonnet + ДООБУЧАТЬ («раньше получалось и сейчас выйдет»). Ставка на детерминированный
# enforce (линтер: хедж-финал, %>100, заголовок-одно-предложение; скелет подачи в промпте; эталон K3 §8.1-b) +
# петлю обучения (write_feedback → scope_lessons), а НЕ на дорогую модель. 2FA — отдельным проходом (verify,
# Sonnet). В /test всё падает на Haiku через runmode. Вернуть Opus — только если дообучение упрётся в потолок.
SCOPE_MODEL = "claude-sonnet-4-6"
# ФИНАЛЬНЫЙ ПОЛИШ ПОДАЧИ — ЕДИНСТВЕННЫЙ вызов дорогой модели. Архитектура «мозги где надо»: всю чёрную работу
# (гейт/поиск/факты/2FA/черновик) делает дешёвый SCOPE_MODEL, а Opus платится ТОЛЬКО за композицию (голос/
# скелет K3), где незаменим — БЕЗ цикла инструментов. Так Opus-подача выходит ~$0.75/прогон вместо ~$1.6
# (полный Opus-цикл 13.07). Причина: Sonnet-черновик на СВЕЖЕМ поводе подачей не дотягивал до K3, а Opus
# генерализует (проверено на AI-капексе). POLISH_MODEL=None — отключить полиш (публикуется Sonnet-черновик,
# ~$0.58, если решим что Sonnet сам тянет). Фолбэк: полиш упал → отдаём Sonnet-черновик, публикацию НЕ рвём.
POLISH_MODEL = None  # ВЫКЛЮЧЕНО 13.07: Opus-полиш дал +$0.56 к прогону («цена x2»), а до уровня владельца
# всё равно не дотягивал — он правит каждый пост руками. Платить за полиш, который всё равно переписывается,
# бессмысленно. Пишет чистый Sonnet-черновик (скелет+линтер помогают бесплатно), финальный голос — владелец.
...
# Полиш-проход работает БЕЗ поиска/гейта — только сохранить финал (линтер прогонит). Даём один инструмент.
_POLISH_TOOLS = [t for t in creator_tools.TOOLS if t.get("name") == "save_draft"]
...
TASK = (
    ...
    "бери ТОЛЬКО длину/тугость тона). Закрывай ОСТРОЙ строкой-выводом (польза-линза, §8.3), не общей ремаркой. ⭐ СКЕЛЕТ ПОДАЧИ (ОБЯЗАТЕЛЕН — из разбора эталонных постов): хук-цифра/факт сразу → разбор фактов с именами → разговорная СВЯЗКА-МОСТ («Веду к тому...» / «Ну вот...») переводит из фактов в интерпретацию → ФИНАЛ = ОБРАЗ-АФОРИЗМ авторским голосом («вцепились за жирный кусок», «очередь начинает двигаться», «долгосрочник докупает»). ⛔ Финал НЕ хедж («ориентир, а не сигнал», «время покажет», «поживём-увидим») и НЕ повтор тезиса своими словами — дай ОБРАЗ, который читатель прокрутит в голове. Каждый факт-контраст подкрепи слоем «ЗНАЧИТ...» (что это значит), не бросай голую цифру. Заголовок-строка + ОДИН эмодзи ПО ТЕМЕ "
...
POLISH = (
    "ФИНАЛЬНАЯ ШЛИФОВКА ПОДАЧИ (scope 🔭). Ниже — ГОТОВЫЙ ПО ФАКТАМ черновик: гейт, цифры, 2FA пройдены. "
    "ПЕРЕПИШИ его в фирменную подачу канала, НЕ меняя факты/цифры/имена/даты. Скелет: хук-цифра → факты с "
    "именами → разговорная СВЯЗКА-МОСТ («Веду к тому...» / «Ну вот...») → ФИНАЛ = ОБРАЗ-АФОРИЗМ авторским "
    "голосом. Простой язык (CPI→«данные по инфляции», спот→«прямые покупки», SOX→«индекс чипмейкеров»). "
    "Каждый факт-контраст со слоем «значит...». ⛔ Финал НЕ хедж («ориентир, а не сигнал», «время покажет»). "
    "Футер — ДОСЛОВНО со ссылками.\n"
    "ЭТАЛОН ТЕКСТУРЫ (бери ПОДАЧУ, не тему): «📉 Продавцы биткоина выдохлись | В июне держатели сбрасывали по "
    "2 000 монет в день - прямо на биржах. В июле поток усох до 53. Разница в 37 раз | Именно так выглядит "
    "ослабление продавцов - паника закончилась | [факты, каждый со слоем «значит...»] | Веду к тому - главное "
    "не X, а Y | Продавцы устают - долгосрочник докупает».\n"
    "Вызови save_draft(kind='scope') с ФИНАЛОМ — МАКСИМУМ ДВА раза (линтер вернул правки → устрани и сохрани "
    "ещё РАЗ, дальше НЕ крути). Затем выведи ТОЛЬКО финальный пост, без преамбулы и меты.\n\nЧЕРНОВИК:\n{draft}"
)
...
def _system_polish() -> str:
    """ЛЁГКИЙ контекст ТОЛЬКО для полиш-прохода: персона + голос (voice_core). БЕЗ мануала, БЕЗ банка
    заголовков (291 шт!), БЕЗ брифа — полиш переписывает ГОТОВЫЙ черновик, ему нужен только голос; скелет
    и эталон K3 вшиты в сам промпт POLISH. Так один вызов ~4к токенов вместо ~40к → Opus-полиш дёшев даже
    при паре итераций с линтером (был баг 13.07: полный контекст × 7 вызовов = $0.56 за один полиш)."""
    persona = config.load_agent(AGENT_NAME)["persona"]
    ctx = ("## 🎙 ГОЛОС/РЕМЕСЛО СКОУПА (voice_core.md) — СТРОГО: типографика §2, анти-ИИ §4, простой язык §7\n"
           f"{_read('memory/voice_core.md')}\n")
    return llm.build_system(persona, ctx)


def _turn(user_text: str, model: str, key: str, thinking, tools: list = TOOLS, system: str = "") -> str:
    text, _ = llm.reply(model, system or _system(), [], user_text, tools, creator_tools.dispatch, key, thinking)
    return text or ""
...
def write(theme: str = "", avoid: str = "", recommend: str = "") -> str:
    ...
    # ФИНАЛЬНЫЙ ПОЛИШ ПОДАЧИ на дорогой модели (POLISH_MODEL) — ЕДИНСТВЕННЫЙ её вызов: переписать готовый по
    # фактам черновик в голос/скелет K3, факты не трогая. Чёрную работу выше сделал дешёвый SCOPE_MODEL. Полиш
    # сам вызывает save_draft (линтер прогонит, драфт перезапишется). Любой сбой → оставляем Sonnet-черновик,
    # публикацию НЕ рвём (фолбэк). POLISH_MODEL=None → полиш выключен (публикуется черновик как есть).
    if POLISH_MODEL:
        try:
            pmodel = runmode.resolve(POLISH_MODEL)
            polished = _turn(POLISH.format(draft=post.split("[[SPLIT]]")[0]), pmodel, key, None,
                             tools=_POLISH_TOOLS, system=_system_polish())
            if polished and polished.strip():
                logging.info("scope: подача отшлифована на %s", pmodel)
                post = polished
        except Exception:
            logging.exception("scope полиш подачи не удался — отдаю Sonnet-черновик")

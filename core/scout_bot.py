...
COMMANDS = {
    "scan": (
        ...
        "свой трек; пересечение AI×крипта — только если ОЧЕВИДНО, не притягивай за уши; "
        "3) по каждому проверь достоверность (ярлык тира требует ссылки; цифра/хайп без "
        "первоисточника → «не подтверждено») и сверь с историей канала (find_posts/by_theme); "

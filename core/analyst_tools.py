...
TOOLS = [
    {
        "name": "edit_report",
        "description": (
            "Что владелец ПРАВИЛ руками: сравнение драфта завода с тем, что реально вышло в канал. "
            "Показывает, сколько текста уцелело (%), какие абзацы сняты/дописаны/переписаны и какие "
            "классы правок ПОВТОРЯЮТСЯ — повторяющийся класс и есть кандидат в уроки, разовая "
            "правка им не является. Зови, когда спрашивают «что не так с форматом», «где мы "
            "недотягиваем», «стало ли лучше»."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "kind": {"type": "string", "description": "формат: 'флагман' | 'scope' (пусто — оба)"},
                "n": {"type": "integer", "description": "сколько последних постов взять (по умолч. 5)"},
            },
        },
    },
    {

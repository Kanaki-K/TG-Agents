...
Дёшево: модель — Sonnet (резолвится через runmode, в /test падает на дешёвую). Вызывается
АВТО-ГЕЙТОМ из /schedule (без чистого вердикта пост в отложку не уходит — см. creator_bot._schedule)
и вручную командой /verify (по последнему драфту).
"""

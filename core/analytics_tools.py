...
_SHARED = {
    ...
    # Замер правок владельца (20.08): драфт завода → то, что реально вышло в канал. Живёт в общем
    # реестре, но СХЕМА пока только у Аналитика: замер — это аналитика формата («где недотягиваем»),
    # а писателю правила приносят уроки и линтер (правило «один вопрос — один хозяин», SCOPE_REWORK).
    # Понадобится Криейтору — схему добавить в его TOOLS, вызов здесь уже готов и идентичен.
    # Считает арифметикой, без вызова модели, поэтому дёргать можно свободно.

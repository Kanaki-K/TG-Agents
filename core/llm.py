...
#
# ⛔ ОТКАЧЕНО В ТОТ ЖЕ ДЕНЬ, 16.09.2026 — ЖИВОЙ ПРОГОН УПАЛ. Новый вариант поиска выполняет
# фильтрацию выдачи ЧЕРЕЗ ИСПОЛНЕНИЕ КОДА под капотом. В одиночном вызове это незаметно, но у нас
# АГЕНТНЫЙ ЦИКЛ: ответ с поиском оставляет «pending tool uses generated by code execution», и
# следующий запрос цикла обязан нести container_id того же контейнера. Наш цикл его не передаёт:
#   400 invalid_request_error: container_id is required when there are pending tool uses
#   generated by code execution with tools
# Прогон 16.09 умер ПОСЛЕ генерации поста (драфт сохранён, 5 замечаний линтера) — на круге правок.
# Пустой кортеж = все роли получают проверенный базовый вариант. Чтобы включить новый, надо сперва
# научить llm.reply возить container_id между шагами цикла и проверить это прогоном, а не тестом:
# тесты сюда не достают — ошибку отдаёт только живой API.
_WEB_SEARCH_MODERN_OK: tuple = ()

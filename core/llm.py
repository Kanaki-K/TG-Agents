...
def reply(model: str, system: str, history: list[dict], user_text: str,
    ...
    while True:
        ...
        params = dict(
            ...
            # TTL=1h (не 5m!): он байт-стабилен в течение дня (analytics.summary детерминирована, дата
            # меняется раз в сутки), поэтому 1-часовой кэш переживает СЕРИЮ прогонов/ходов с паузами —

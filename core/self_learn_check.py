...
(прогони refresh_threads.py) — иначе линковать нечего.

Это ДИАГНОСТИКА (read-only). Сама интеграция ПОДКЛЮЧЕНА 17.07: боевой пикер флагмана
(run_pipeline._pick_timely_theme → self_learn.tg_category_weights) наклоняет ротацию тем по
ТГ-данным (НЕ Threads — площадки инвертированы). Скрипт показывает тот же наклон, что применяет пикер.
"""
...
def main() -> None:
    ...
    print("\nИнтеграция ПОДКЛЮЧЕНА (17.07): пикер флагмана наклоняет выбор темы по ТГ-сигналу выше. "
          "Threads-половина петли — дормант (нет данных). Это диагностика, контент не меняет.")

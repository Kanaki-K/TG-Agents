    ...
    def _make_action(cmd: str, fn):
        ...
        async def handler(m: Message) -> None:
            ...
            try:
                ...
                # Формулировка нейтральная: среди действий есть и минутные прогоны (/run), и мгновенные
                # справки (/autopilot) — «идёт прогон» на справке читалось бы как «тратятся деньги».
                await m.answer(f"⏳ Взял в работу /{cmd} — пришлю результат сюда, как будет готово.")
                await m.bot.send_chat_action(m.chat.id, "typing")
                if takes_arg:

...
def _lint(content: str, kind: str = "") -> tuple:
    ...
    if "scope" in k or "коротк" in k:
        ...
        # ЗАГОЛОВОК — РОВНО ОДНО ПРЕДЛОЖЕНИЕ (владелец, важное): либо утверждение, либо вопрос, НЕ два.
        if _title_two_sentences(head):
            warns.append("scope: ЗАГОЛОВОК из ДВУХ предложений — оставь РОВНО ОДНО (утверждение ЛИБО вопрос). "
                         "❌ «Майнеры продали лопаты. Но не уходят», ❌ «утверждение. вопрос?». Одно предложение — точка")

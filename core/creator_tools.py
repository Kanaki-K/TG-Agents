...
def _lint(content: str, kind: str = "") -> tuple:
    ...
    if "scope" in k or "коротк" in k:
        # ТГ-ONLY формат: считаем ВЕСЬ пост (с футером). Дуал ТГ+Threads откачен 13.07 — Threads будет
        # ОТДЕЛЬНЫМ форматом; тесная ≤500 душила денежную механику (Chainlink не влезал) и гнала стаккато,
        # потому скоуп снова дышит до ~1000 (как было до 13.07 — состояние, что дало удачные посты).
        n = len(clean.encode("utf-16-le")) // 2
        if n > 1000:
            warns.append(f"⛔ scope ДЛИННЫЙ: {n} знаков — потолок ~1000, цель ~800 (тело ~550–650 + футер): scope = "
                         f"БЫСТРАЯ реакция, один поворот + механика с пейоффом. Срежь лекцию/разбор (мимо целевой = мини-флагман)")
        elif n < 350:
            warns.append(f"scope коротковат: {n} знаков — телеграмма. Дай один поворот + цифру-механику + угол (см. §8)")
        # ЗАГОЛОВОК — РОВНО ОДНО ПРЕДЛОЖЕНИЕ (владелец 13.07): либо утверждение, либо вопрос. НЕ два —
        # ни «утверждение. вопрос?», ни «утверждение. приколка». Тег-рубрику «🌐 LINK | …» не считаем.
        head_line = clean.lstrip().split("\n", 1)[0].replace("**", "").strip()
        hl = head_line.split("| ", 1)[-1].strip()
        hl_core = hl.rstrip("?!.… ")                       # финальный знак — не разделитель, снимаем
        if re.search(r"[.!?]\s+\S", hl_core):              # внутренний конец предложения + продолжение = ДВА
            warns.append("scope: ЗАГОЛОВОК из ДВУХ предложений — оставь РОВНО ОДНО (либо утверждение, либо "
                         "вопрос). ❌ «утверждение. вопрос?» и ❌ «утверждение. приколка» (напр. «Robinhood "
                         "строил блокчейн. Кто пришёл первым?»). Одно предложение — точка")
        ...
        # ФИНАЛ = образ-афоризм, НЕ хедж (главный убийца качества scope, урок 13.07). Детектим хедж-концовки
        # детерминированно — промпту это не доверяем (его игнорят). Берём последнюю строку тела (до футера).
        _body_fin = clean.split("🖥")[0].rstrip()
        _last = _body_fin.rsplit("\n", 1)[-1].strip().lower() if _body_fin else ""
        _hedge = ("ориентир, а не сигнал", "ориентир а не сигнал", "время покажет", "покажет время",
                  "поживём", "поживем", "посмотрим что будет", "остаётся ждать", "остается ждать", "время рассудит")
        if any(h in _last for h in _hedge):
            warns.append("scope: ФИНАЛ — ХЕДЖ-концовка вместо образа-афоризма. Дай ОБРАЗ, который читатель "
                         "прокрутит в голове («долгосрочник докупает», «очередь начинает двигаться», «вцепились "
                         "за жирный кусок»), НЕ «поживём-увидим» и НЕ повтор тезиса своими словами (§8.1, урок 13.07)")
        # Проценты >100% читатель не считает на лету → «в N раз» (урок 13.07: «+1800%» → «в 18 раз»).
        _pct = re.search(r"\+?\s?(\d{3,})\s?%", clean)
        if _pct:
            warns.append(f"scope: «{_pct.group(0).strip()}» — процент >100% нечитаем на лету, переведи в "
                         f"«в N раз» (напр. +1800% → «в 18 раз»)")
        ...
        filler = [p for p in ("важнее, чем кажется", "важнее чем кажется", "стоит отметить",
                              "интересно, что", "интересно то, что", "нельзя не отметить",
                              "но под шумом", "под шумом", "под шумок") if p in scl]
        if filler:
            warns.append("scope: ФИЛЛЕР-ПОДВОДКА (нашёл «" + filler[0] + "») — режь, давай суть сразу. "
                         "Подача scope — без раскачки и канцелярских подводок.")
        # СЛОВА-КОСТЫЛИ scope — заезжены по постам (модель тянет одно и то же). «рельса/рельсы» — самый
        # частый (SWIFT, Robinhood ×2…): в 10-й раз это ИИ-штамп, найди другой образ.
        crutch = [p for p in ("рельс",) if p in scl]
        if crutch:
            warns.append("scope: СЛОВО-КОСТЫЛЬ «" + crutch[0] + "…» (рельса/рельсы) — заезжено по постам, "
                         "стало ИИ-штампом. Найди ДРУГОЙ образ (инфраструктура, канал, труба, магистраль — "
                         "или вообще без метафоры).")
    return clean, warns

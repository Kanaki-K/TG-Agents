...
def _lint(content: str, kind: str = "") -> tuple:
    ...
    # §5/voice_core §2: ТОЧКА в конце СТРОКИ — фирменный маркер КОРОТКОГО формата (scope/short): 93%
    # концовок автора без точки. Срезаем ДЕТЕРМИНИРОВАННО ТОЛЬКО там. Флагман этого авто-среза в рабочую
    # эпоху (посты до 26.06) НЕ имел — его концовки не трогаем (автор местами ставит точку осознанно,
    # напр. «51-56к.» в утверждённом посте). Бережём «...» (троеточие), «?»/«!» и точку МЕЖДУ предложениями.
    _k0 = (kind or "").lower()
    if "scope" in _k0 or "коротк" in _k0 or "short" in _k0:
        _pl = []

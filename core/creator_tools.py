...
# ЗАМЕР (14.08): 0 совпадений на 291 заголовке банка и 0 на последних 22 постах канала — то есть
# приём в утверждённой подаче не встречается ВООБЩЕ, ложных срабатываний ждать неоткуда. Обычную
# запятую НЕ трогаем: «Сеть Circle обслуживают те же, кто в неё вложил деньги» и «Кофейня, которая
# покупает биткоин» — одна мысль с придаточным ВНУТРИ, они канону не противоречат.
...
def _title_keep_first(head: str) -> str:
    ...
    if _TITLE_FRONT_SUB.match(_rest.strip()):

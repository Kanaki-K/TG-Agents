...
def _lint(content: str, kind: str = "") -> tuple:
    ...
    # Это СЕТЬ-предупреждение (видно в консоли) — переписывает редактор (POLISH); имена/тикеры/компании
    ...
    if "scope" in k or "коротк" in k:
        ...
        # Потолок КАЛИБРОВАН по принятым постам (замер: Chainlink 1079 / SWIFT 1203 / BNY 1216 / Orange 1276
        # всего). Старый ⛔ >1000 бил по КАЖДОМУ хорошему посту → писатель резал живую мысль в обрубок. Провал
        # формата = раздувание >1350 (мини-флагман, урок 24.07 про 1770); коридор 1050–1350 = норма.
        if n > 1350:
            warns.append(f"⛔ scope ДЛИННЫЙ: {n} знаков — потолок ~1350 всего (цель тело ~850–1050 + футер): scope = "
                         f"ОДИН поворот + одна цифра, не мини-флагман. Срежь ВОДУ/лекцию/повтор — но НЕ едж и не спину")
        elif n < 500:
            warns.append(f"scope коротковат: {n} знаков — телеграмма/обрубок (принятые посты 1080–1280). Доведи "
                         f"механику (было→стало) + поворот + самостоятельный финал (скелет §6)")

...
# Якорим на ПРОВЕНАНС-префиксы (из правки / из разбора / из поста / владелец… / правка владельца) —
# НЕ на любой `— _italic_`. Иначе будущий урок с inline-курсивом-правилом в конце строки («держи
# `_жёстко_`») был бы срезан как «хвост» → потеря ПРАВИЛА. Покрывает все текущие хвосты (36/18).
_LESSON_EVIDENCE_RE = re.compile(r"\s*[—–-]\s*_(?:из |владелец|правка).+_\s*$")

...
def _lint(content: str, kind: str = "") -> tuple:
    ...
    if "scope" in k or "коротк" in k:
        # ДУАЛ ТГ+Threads: считаем именно ТЕЛО (без футера, без ** markdown) — это то, что репостится
        # в Threads, у которого жёсткий лимит 500 знаков. Футер (🖥…Notion) — ТГ-only фурнитура, в счёт
        # не идёт; в Threads он и не поедет (ссылки режут охват x5). Цель формата: одно тело на 2 площадки.
        body = clean
        fi = body.find("🖥")
        if fi != -1:
            body = body[:fi]
        body_vis = body.replace("**", "").strip()
        n = len(body_vis.encode("utf-16-le")) // 2
        if n > 500:
            warns.append(f"⛔ scope ДЛИННЫЙ: тело {n} знаков — ЛИМИТ 500 (тело без футера/**). Threads не примет "
                         f">500, дуал ломается. scope = БЫСТРАЯ реакция, ОДИН поворот + одна цифра — срежь "
                         f"лекцию/мехразбор до ≤500 (мимо = мини-флагман)")
        elif n > 460:
            warns.append(f"scope у стены: тело {n} знаков (лимит 500) — подожми запас, чтобы гарантированно "
                         f"влезло в Threads при репосте")
        elif n < 200:
            warns.append(f"scope коротковат: тело {n} знаков — телеграмма. Дай один поворот + цифру-механику + угол (см. §8)")

...
def _title_keep_first(head: str) -> str:
    """Оставить в заголовке РОВНО ПЕРВОЕ предложение (владелец 20.07 СТРОГО: одно утверждение ЛИБО
    вопрос). Раньше правило было только warn — модель его РЕГУЛЯРНО переспоривала («авторское решение,
    не режу», прогоны 24.07). Режем ДЕТЕРМИНИРОВАННО в КОДЕ, как тире/кавычки: промпту тут не верим.
    Бережём жирную обёртку **…**, ведущий эмодзи-якорь, тег-рубрику «🌐 LINK | …» (режем только текст
    ПОСЛЕ разделителя) и инициал «T. Rowe»/«U.S.». Один поворот — возвращаем head без изменений."""
    raw = head.strip()
    ...
    if not m:
        return head
    first = inner[:m.start() + 1].rstrip()               # включаем сам знак конца предложения
    if first.endswith(".") and not first.endswith(".."):  # точка в конце заголовка не ставится (§5)
    ...
    if "scope" in k or "коротк" in k:
        ...
        if _title_two_sentences(head):
            ...
            if _one and _one != head:
                ...
                clean = "\n".join(_ls2)
                head = _one
                warns.append("scope: заголовок был из ДВУХ предложений — оставил РОВНО первое "
                             "(детерминированно, владелец 20.07). Проверь, что крючок не потерялся.")

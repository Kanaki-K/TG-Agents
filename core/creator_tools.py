...
def _lint(content: str, kind: str = "") -> tuple:
    ...
    if "scope" in k or "коротк" in k:
        ...
        if n > 1000:
            warns.append(f"⛔ scope ДЛИННЫЙ: {n} знаков — потолок ~1000, цель ~800 (тело ~550–650 + футер): scope = БЫСТРАЯ "
                         f"реакция, ОДИН поворот + одна цифра. Срежь лекцию/разбор (мимо целевой = мини-флагман)")
        elif n < 350:

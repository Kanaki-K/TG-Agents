...
def _lint(content: str, kind: str = "") -> tuple:
    ...
    if "scope" in k or "коротк" in k:
        ...
        # Тег-рубрику «🌐 LINK | …» в начале не считаем частью предложения. Финальный знак снимаем.
        _hl = clean.lstrip().split("\n", 1)[0].replace("**", "").strip()
        _hl = _hl.split("| ", 1)[-1].strip().rstrip("?!.… ")
        # Инициал/сокращение из ОДНОЙ буквы + точка («T. Rowe Price», «U.S.», «т. д.») — это НЕ конец
        # предложения: снимаем их перед проверкой, иначе точка после «T» даёт ложный «два предложения» (17.07).
        _hl_probe = re.sub(r"\b[A-Za-zА-Яа-яЁё]\.", "", _hl)
        if re.search(r"[.!?]\s+\S", _hl_probe):   # внутренний конец предложения + продолжение = ДВА предложения

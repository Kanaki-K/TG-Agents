...
def _make_image(args: dict) -> str:
    ...
    return (
        ...
        "ТЕПЕРЬ выведи в чат РОВНО ОДИН раз ТОЛЬКО финальный пост — на 100% готовый к публикации "
        "(копировать и постить). СТРОГО без лишнего, это перекрывает обычный формат вывода с метой:\n"
        "• НЕ упоминай картинку («обложка ушла ⬆️» и т.п.) — её шлёт система, не дублируй;\n"
        "• НИКАКОЙ преамбулы перед постом («Готово», «Финальная версия», «Поправил линтер…» и т.п.) — "
        "первая строка ответа = первая строка ПОСТА;\n"
        "• НЕ давай меты, оценок X/10, [[SPLIT]], списка «что поправил», советов и вопросов;\n"
        "• мелкие правки (типографика, канон-футер) внеси МОЛЧА прямо в текст;\n"
        "• не повторяй пост дважды.\n"
        "Один пост — и всё. Если хочешь что-то предложить владельцу — дождись, пока он сам спросит."
    )

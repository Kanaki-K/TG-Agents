...
# ── МАРШРУТ КАДРА: ЧЕМ ОН ОКАЗАЛСЯ, А НЕ ТОЛЬКО ОТКУДА ВЗЯТ (07.09) ──────────────────────────────
# Судье мало показать восемь картинок: по замеру 23 обложек у канала есть ПОРЯДОК предпочтения
# (снятый объект → полотно бренда → человек → предмет → иллюстрация издания), а на глаз отличить
# «фирменное полотно» от «редакционной иллюстрации» можно не всегда. Мы-то знаем точно: полотно
# пришло с официального сайта объекта, фото — из Wikidata/Commons, лого — из P154. Эту метку и
# передаём в промпт, чтобы порядок из мануала был исполним, а не пожеланием.
# Повод: 07.09 в пуле лежали и фирменное полотно Liquid Network, и редакционный коллаж издания.
# Судья дважды подряд выбрал коллаж, хотя владелец руками поставил именно полотно — а по замеру
# полотен на канале 6, иллюстраций изданий 2.
_KIND: dict[str, str] = {}


def _route_of(what: str) -> str:
    """Техническое описание находки → маршрут из memory/scope_cover_manual.md §3."""
    if what.startswith("бренд-полотно"):
        return "фирменное полотно бренда с официального сайта (маршрут 3.2)"
    if what.startswith("лого"):
        return "голое лого — страховка, самый слабый вариант"
    return "снятый объект/человек из открытых источников (маршрут 3.1/3.3)"


def kind_of(url: str) -> str:
    """Маршрут, которым найден кадр. Пусто — кадр не из поиска по объекту."""
    return _KIND.get(url, "")
...
def subject_image_urls(subject: str, limit: int = 4, page_urls: list | None = None) -> list[str]:
    ...
    # ГОЛЫЕ ЛОГО ИЗ СТРАХОВКИ УБРАНЫ (07.09). Бакет `plain` — это P154 и поиск «X logo» на Commons,
    # то есть ровно «низкокачественный PNG-логотип на пустом фоне», который владелец внёс в стоп-лист
    # (§5 п.8): «просто логотип на белом фоне» он забраковал ещё 31.08, а 07.09 назвал прямо. Среди 23
    # принятых обложек голого лого нет ни одного — лого живёт только внутри фирменного полотна (§3.2),
    # а полотно приходит бакетом `rich` с официального сайта. Пустой пул честнее плохой обложки:
    # дальше включается второй круг по кадрам со статей.
    out = rich[:limit]
    if not out:

"""Нативная (отложенная) публикация поста в Telegram-канал через MTProto (аккаунт-публикатор).

Это «руки» Публикатора. Кладём пост аккаунтом-владельцем (userbot), а НЕ Bot API: у userbot нет
лимитов Bot API, пост ложится в канал как руками человека (обложка сверху + полный текст).
...
data/mtproto.session) — один аккаунт, две функции (read метрики + write публикация), логика раздельна.

Доставка — ФАЙЛОМ, без URL/превью/хостинга (хостинг обложки и превью-ссылки выпилены 20.06 — ломались):
  1) с обложкой, текст влез в подпись → фото-файл + текст-подпись ОДНИМ сообщением;
  2) с обложкой, подпись длиннее лимита Telegram (MediaCaptionTooLongError) → фото + текст ДВУМЯ
     отложенными сообщениями (пост не теряем);
  3) без обложки → просто текст.

Рендер как у Криейтора в чате: жирный (**…**→bold) + кастом-эмодзи (<tg-emoji> из data/custom_emoji.json,
премиум-аккаунт их шлёт) через tg_format.to_telegram_html → parse_mode="html", link_preview=False; при
сбое разметки фолбэк на чистый текст (parse_mode=None). Синхронные обёртки гоняют Telethon в своём event
loop — их зовут из потока бота (asyncio.to_thread), как telegram_scan.recent.
"""

...
# Темп ускорен 17.07: прежние 4-11с + передышки 30-120с давали ~20 мин на рядовой сбор (~65
# запросов) — неприемлемо для сбора ~ежедневно. Прежняя медлительность шла частью из «не выглядеть
# машиной» (снято 15.07) — осталась только вежливость. ~0.3-0.7 запр/сек — вежливо и не близко к
# per-second лимиту Meta (613); настоящий тормоз — стоп по квоте x-app-usage 80%, он не тронут.
GAP_MIN = float(os.getenv("THREADS_GAP_MIN", "1.5"))
GAP_MAX = float(os.getenv("THREADS_GAP_MAX", "4.0"))
# Раз в N запросов — пауза подлиннее: длинная пагинация не должна идти сплошной очередью.
BREATHER_EVERY = (25, 40)
BREATHER_SECONDS = (8.0, 20.0)

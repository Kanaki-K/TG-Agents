...
def _threads_followers_daily() -> None:
    """Снять счётчик подписчиков Threads раз в сутки. Один запрос ЧТЕНИЯ, и он не ждёт.

    ЗАЧЕМ (замер 09.09.2026): подписок НА ПОСТ Meta не отдаёт вообще, а followers_count существует
    только как «сколько сейчас» — временного ряда у него нет и задним числом он не восстанавливается.
    Значит ряд надо копить самим, и копить ЕЖЕДНЕВНО: пропущенный день — дырка в атрибуции навсегда.
    Автопилот единственный, кто просыпается каждый день, поэтому снимок висит на нём.

    Сбои и повторные вызовы берёт на себя сам модуль (снимок один на дату, ошибки глотает)."""
    try:
        from connectors.threads import _guard
        if _guard.frozen_reason():        # сеть Threads закрыта стоп-краном — ходить нельзя
            return
        from core import threads_followers
        threads_followers.snapshot_quiet()
    except Exception:  # noqa: BLE001 — фоновая гигиена не роняет прогон
...
def _threads_insights_intake() -> None:
    ...
    Проверяем папку каждый день, но файлы там появляются раз в 3-5 суток — пустая папка это штатный
    день, а не сбой, поэтому молчим."""
    try:
...
def _factory_baseline_monthly() -> None:
    """Раз в месяц снять точку отсчёта и прислать владельцу, что изменилось.

    ЗАЧЕМ АВТОМАТОМ. Владелец про ручной запуск сказал прямо: «вот это я не запомню, точнее не
    понимаю, для чего мне это через месяц». Он прав дважды. Во-первых, инструмент, который надо
    помнить, не работает. Во-вторых, польза видна не в момент замера, а через месяц — когда
    приходит сравнение и видно, сработали правки или нет. Поэтому снимок делает завод, а владелец
    получает готовый ответ в Telegram: подписчики, воронка, рычаг ставки — было и стало.

    Раз в месяц, потому что метрики меняются медленно: недельная разница в охвате это шум
    площадки, а не результат наших правок. Сбой глотаем — замер не имеет права уронить выход поста.
    """
    try:
        from core import factory_baseline
        took, text = factory_baseline.take()
        if took and text:
            log.info("[автопилот] снята точка отсчёта:\n%s", text)
            bot_alert.notify_owner(text)
    except Exception:  # noqa: BLE001 — фоновая гигиена не роняет прогон
        log.warning("[автопилот] точку отсчёта снять не смог", exc_info=True)


def _threads_daily_hygiene() -> None:
    """Ежедневная гигиена аккаунта Threads — НЕ зависит от того, включён ли автопилот на выход.

    Почему до проверки выключателей: обе задачи (живой токен и ряд подписчиков) ломаются именно от
    простоя. Токен уже умер один раз ровно так — с 17.07 в API не ходил никто, окно продления
    закрылось, и 31.08 понадобился OAuth руками. Ряд подписчиков рвётся так же тихо: выключил
    автопилот на неделю — в атрибуции неделя дырок, и восстановить её нечем. Гигиена дешевле
...
def check_once() -> str:
    ...
    """
    _threads_daily_hygiene()   # до выключателей: токен и ряд подписчиков ломает именно простой

...
def _run_creator(command: str = "post", avoid: str = "", hint: str = "", theme: str = "",
    ...
    if evergreen:  # ВЕЧНАЯ ТЕМА (Модель А): тему ВЫБРАЛ пикер, Криейтор её РАСКРЫВАЕТ (не выбирает из 200).
        guard = ("Пиши образовательный флагман на ВЕЧНУЮ тему (не новость). Тема ВЫБРАНА за тебя из банка:\n"
                 f"  {theme}\n"
                 + (f"\n⭐ УГОЛ ВХОДА (v2, выбран пикером под сегодняшний рынок): {angle}\n"
                    "Заходи ИМЕННО так. Это не украшение: замер 63 постов показал, что тему убивает не "
                    "она сама, а вход в неё — объяснялки устройства дали ноль заходов в профиль, а те же "
                    "темы, поданные через сцену и потерю, стали лучшими постами квартала.\n"
                    "ГЕЙТ ЧЕТЫРЁХ ЭЛЕМЕНТОВ (мануал §4): в посте минимум ДВА из четырёх — герой, сцена, "
                    "потеря/ставка, парадокс. Заголовок — УТВЕРЖДЕНИЕ С ПЕРЕВОРОТОМ, не вопрос.\n"
                    "⭐ ГЕЙТ ВЫХОДА (важнее гейта входа): перед сдачей ответь себе — ЧТО ЧИТАТЕЛЬ "
                    "СМОЖЕТ СДЕЛАТЬ ИЛИ ПОДУМАТЬ ЗАВТРА ИНАЧЕ, конкретно? «Теперь он знает про X» — "
                    "это НЕ ответ, это иллюзия знания. Нечего ответить — дай инструмент (вопрос, "
                    "проверку, критерий выбора) или честно скажи владельцу, что пост даёт понимание, "
                    "а не защиту. В мете после [[SPLIT]] обязательны строки [[УЗЕЛ]], [[ТИП]], "
                    "[[ВЫХОД]] — они не публикуются.\n"
                    if angle else "")
...
def _pick_timely_theme() -> tuple[str, str, dict, str]:
    ...
    system = (
        ...
        "4) ДРАМА (v2, решающий критерий): в теме должно найтись минимум ДВА элемента из четырёх — "
        "ГЕРОЙ (лицо, за которым следят), СЦЕНА (конкретный случай, а не «механизм устроен так»), "
        "ПОТЕРЯ/СТАВКА (кто теряет и на сколько), ПАРАДОКС (обещанное разошлось с настоящим). Тема "
        "без драмы даёт пост, который читают и забывают: замер 63 постов — ноль заходов в профиль у "
        "объяснялок устройства и справок. ⚠️ Драму ИЩИ В ТЕМЕ, а не отбрасывай тему: «AI-оракулы» "
        "звучат скучной инфраструктурой, а стали лучшим постом квартала, когда их подали как "
        "ограбление биржи на 110 млн$. Почти в каждой теме зарыта история или человеческое решение.\n"
        ...
        "бриф), УГОЛ — одна фраза, чья это драма, кто теряет и с какой сцены заходить. "
        "Пример: `7 | рынок | биржу обчистили на 110 млн$ без единой взломанной строчки, теряют "
        "вкладчики — заходим со сцены сделки | механизм`."
    )
...
def run_cycle(scope: bool = False, skip_scout: bool = False, draft_only: bool = False,
    ...
    if not scope:
        ...
        out(f"   └→ УГОЛ (v2): {theme_angle or 'пикер не дал — писатель ищет сам'}\n")
        if theme_angle:

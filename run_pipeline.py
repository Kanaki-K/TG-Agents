...
def run_cycle(scope: bool = False, skip_scout: bool = False, draft_only: bool = False,
    ...
    if scope:
        ...
        try:
            # Ре-разведка на ИСЧЕРПАНО (только 🚫-поток/повтор) ИЛИ ОФФ-БРЕНД (осталась только чужая тема —
            # мемкоины/шум, «что НЕ наше» brand.md; баг 24.07: гейт был слеп к бренду → опубликовал DOGE+SHIB).
        ...
        # БРЕНД-ВЕТО (владелец 24.07): если и ПОСЛЕ разведки всё свежее — офф-бренд (мемкоины/шум/сигналы),
        # он-бренд темы нет вообще → ПРОПУСКАЕМ прогон. Бренд СИЛЬНЕЕ «пост обязан быть»: лучше нет поста,
        # чем чужой жанр (как красные линии — тему про Россию тоже не публикуют ради галочки). Не тихо —
        # сообщаем владельцу. Поста не будет, в канал ничего не уйдёт.

...
def _run_creator(command: str = "post", avoid: str = "", hint: str = "", theme: str = "",
                 evergreen: bool = False, no_image: bool = False, angle: str = "") -> str:
    ...
    if evergreen:  # ВЕЧНАЯ ТЕМА (Модель А): тему ВЫБРАЛ пикер, Криейтор её РАСКРЫВАЕТ (не выбирает из 200).
        guard = ("Пиши образовательный флагман на ВЕЧНУЮ тему (не новость). Тема ВЫБРАНА за тебя из банка:\n"
                 f"  {theme}\n"
                 + (f"\n⭐ УГОЛ (v3, ориентир пикера — не сценарий): {angle}\n"
                    "Угол говорит, в чём драма темы и на каком материале она держится. Вход в пост — "
                    "ТОЛЬКО реальный материал: событие с датой, исследование с авторами и цифрами, "
                    "человек с именем, документ (мануал §4 «Вход»). Сцену не сочиняй и не приписывай "
                    "читателю, что он делал. Не цепляй вечную тему к цене этой недели и не обещай, что "
                    "цена вернётся.\n"
                    "ГЕЙТ ВЫХОДА: перед сдачей ответь себе, что читатель сможет сделать или подумать "
                    "завтра иначе, конкретно («теперь он знает про X» — иллюзия знания, не ответ). Это "
                    "проверка разбора, а не лишний блок в посте. В мете после [[SPLIT]] "
                    "обязательны строки [[УЗЕЛ]], [[ТИП]], [[ВЫХОД]] — они не публикуются.\n"
                    if angle else "")
                 + "Раскрой ИМЕННО ЕЁ (и её угол после тире) — не бери другую, не изобретай свою.\n"
                 "ЗАЯКОРИ на КОНКРЕТИКЕ: реальный пример/кейс с датой, исследование, историческая "
                 "параллель — не абстрактное «представьте, что Вы…». Живая цена (market_price) — только "
                 "если разбор про рынок сейчас.\n"
...
def _pick_timely_theme() -> tuple[str, str, dict, str]:
    """Тема флагмана по АКТУАЛЬНОСТИ. Возвращает (тема, ФАКТОР_РЕШЕНИЯ, вход_измеримо, УГОЛ).

    УГОЛ (v2, 10.09.2026) — чья это драма и кто теряет, одной фразой. Почему угол выбирается ЗДЕСЬ,
    а не прописан в банке заранее: замер показал, что убивает не тема, а вход в неё («AI-оракулы»
    стали лучшим постом квартала, когда их подали как ограбление). Угол зависит от сегодняшнего
    рынка и брифа — статическая строка в банке устарела бы к следующему разу, а 210 углов руками
    были бы 210 догадками вслепую. Пикер видит рынок, бриф и покрытие канала — он и решает.
    ...
    system = (
        ...
        "4) ДРАМА (v3 — ранжирует, не вето): выше тема, где в РЕАЛЬНОМ материале есть герой (лицо), "
        "случай с датой, потеря с числом или парадокс (обещанное разошлось с настоящим). Драму ИЩИ В "
        "ТЕМЕ: «AI-оракулы» стали ограблением биржи на 110 млн$. Но драма не выдумывается: нет реального "
        "случая — вход через исследование или механизм, а не через сочинённую сценку.\n"
        "5) ТИП УСЛУГИ (v2.1): у поста один ведущий тип — механизм | личная ставка | инструмент | "
        "переворот | линза. ДВА ПОСТА ПОДРЯД ОДНОГО ТИПА ЗАПРЕЩЕНЫ: канал перестаёт делать читателя "
        "способнее по-новому и начинает повторять одну услугу. Последние типы указаны ниже — выбери "
        "тему, из которой естественно вырастает ДРУГОЙ тип, и назови его в ответе.\n"
        "Ответ СТРОГО в формате `номер | фактор | угол | тип`: фактор — ОДНО слово (свежесть | рынок | "
        "бриф), УГОЛ — одна фраза: в чём переворот или кто теряет И на каком реальном материале это "
        "держится (событие с датой, исследование, человек, документ). ⛔ Не сцена «человек весь вечер "
        "смотрит график» и не цена этой недели: фактор «рынок» выбирает ТЕМУ под настроение, а в угол "
        "цену недели не тащи. "
        "Пример: `7 | рынок | Mango Markets обчистили на 110 млн$ через подкрученный оракул "
        "(11.10.2022) — теряют вкладчики | механизм`."
    )
    recent_srv = published_journal.recent_services(3)
    user = (f"ТЕМЫ:\n{numbered}\n\n"
            f"ТИПЫ УСЛУГИ ПОСЛЕДНИХ ПОСТОВ (свежий первый): "
            f"{', '.join(recent_srv) if recent_srv else 'нет данных'} — повторять последний нельзя\n\n"
            f"ЧТО КАНАЛ УЖЕ ПУБЛИКОВАЛ (не повторять ПО СУТИ):\n{coverage}\n\n"
    ...
    try:
        ...
        if nums and 0 <= int(nums[0]) - 1 < len(sample):
            parts = raw.split("|")
            angle = parts[2].strip() if len(parts) > 2 else ""
            srv = parts[3].strip() if len(parts) > 3 else ""
            if srv:
                # Тип услуги идёт писателю ВМЕСТЕ с углом: он определяет, чем пост кончится —
                # инструментом, линзой или пониманием. Гейт выхода (мануал) проверяет именно это.
                angle = f"{angle}\n   ТИП УСЛУГИ: {srv} — пост обязан оставить читателя способнее ИМЕННО так"
                meas["тип услуги"] = srv
            return sample[int(nums[0]) - 1], factor, meas, angle
    except Exception:  # noqa: BLE001
...
def run_cycle(scope: bool = False, skip_scout: bool = False, draft_only: bool = False,
    ...
    try:
        _kind_ru = "scope" if scope else "флагман"
        _delta = edit_delta.panel_line(_kind_ru)
        if _delta:
            out(f"📐 Прошлый пост этого формата после твоей правки: {_delta}\n")
            panel["📐 правки"] = _delta
        # ВОПРОС ВЛАДЕЛЬЦУ (09.09): процент показывает ЧТО и СКОЛЬКО, но почему — знает только он.
        # Спрашиваем, лишь когда класс правки ПОВТОРИЛСЯ, и всегда с цитатами: «11 из 15 правлю
        # заголовок» лечится не новым числом в панели, а одной его фразой, ставшей уроком.
        _q = edit_delta.ask(_kind_ru)
        if _q:
            out(_q + "\n")
    ...
    theme = theme_why = theme_angle = ""
    if not scope:
        theme, theme_why, meas, theme_angle = _pick_timely_theme()
        if not theme:
        ...
        out(f"   ├ тип услуги (ротация) .... {meas.get('тип услуги', '— пикер не назвал')}")
        out(f"   ├→ РЕШЕНИЕ: «{theme}»   ПОЧЕМУ: {theme_why}")
        out(f"   └→ УГОЛ (v3): {theme_angle or 'пикер не дал — писатель ищет сам'}\n")
        if theme_angle:
            panel["🎭 угол"] = theme_angle[:80]
    ...
    if scope and post:
        ...
        try:
            ...
            _unver = verify.unverified_notes(sv)          # ❓ «в брифе не прослеживается» — тоже владельцу
            ...
            if verify.has_issues(sv) and verify.only_completeness(sv):
                ...
                elif verify.has_mechanics(sv2):
                    # КОНФЛИКТ МЕХАНИКИ — НЕ «числовой нюанс» (вред 05.08). Цифры целы, а устройство
                    # описано неверно: «расчёт в USDC вместо T+2» (на деле расчёт ВНЕ депозитария) и
                    # «Circle стейкует» в настоящем времени (мейннет только 16 сентября). Оба ушли в
                    # отложку под чужим ярлыком, потому что код знал ровно два класса: красная линия и
                    # «остаточный нюанс». Неточная цифра читателя не обманывает, неверный механизм —
                    # обманывает, ради него пост и написан. Даём ОДИН прицельный круг (без веба, ~$0.04)
                    # и проверяем КОДОМ, что формулировка реально изменилась: механику чинят
                    # переписыванием, поэтому уцелевшая дословно строка = правка не состоялась.
                    # Стопать нельзя (правило 22.07 «пост обязан быть») — но и врать в панели тоже.
                    out("🔧 2FA: цифры целы, но МЕХАНИЗМ описан неверно (право подано как действие / "
                        "«вместо» там, где «вне» / выгода не тому участнику) — переписываю прицельно:")
                    _mt = verify.mechanics_targets(sv2)
                    post = _threaded(scope_writer.fix_facts, verify.strip_completeness(sv2), fkey) or post
                    _stuck = verify.targets_left((post or "").split("[[SPLIT]]")[0], _mt)
                    if _stuck:
                        panel["🔎 2FA scope"] = ("⚠️ механика НЕ переписана (" + _clip("; ".join(_stuck), 34) +
                                                ") — глянь до одобрения")
                        out("⚠️ Формулировка осталась ДОСЛОВНО прежней — правка не состоялась: " +
                            "; ".join(_stuck) + "\nПроверь в «Отложенных», прежде чем одобрять.\n")
                    else:
                        panel["🔎 2FA scope"] = "механизм описан неверно — переписал (проверено кодом); глянь"
                        out("✅ Механизм переписан по источнику (формулировка изменилась):\n" + str(sv2) + "\n")
                elif verify.has_issues(sv2):
                    # ОСТАТОЧНЫЙ ЧИСЛОВОЙ ⚠️ (не красная линия, не механика) — не стопаем (урок 22.07: стоп
                    # на педантичном нюансе «The Open Platform ≠ TON Foundation» = $1.3 и ноль).
                    # НО И НЕ ПУБЛИКУЕМ МОЛЧА (14.09): вторая сверка назвала «12-13%» при проверенных 11.8%, и
                    # цифра уехала в отложку нетронутой. Даём ОДИН прицельный круг по этому вердикту (без
                    # третьей веб-сверки — решение 31.07 в силе) и КОДОМ проверяем, что фраза изменилась.
                    _nt = verify.numeric_targets(sv2)
                    if _nt:
                        out("🛠 2FA: после правки осталась неточная цифра — правлю прицельно по веб-значению "
                            "из вердикта (без третьей сверки):")
                        post = _threaded(scope_writer.fix_facts, verify.strip_completeness(sv2), fkey) or post
                    _nstuck = verify.targets_left((post or "").split("[[SPLIT]]")[0], _nt)
                    if _nt and not _nstuck:
                        panel["🔎 2FA scope"] = "остаточная цифра поправлена по вебу (проверено кодом); глянь"
                        out("✅ Остаточная цифра переписана по веб-значению:\n" + str(sv2) + "\n")
                    else:
                        panel["🔎 2FA scope"] = ("⚠️ цифра НЕ поправлена (" + _clip("; ".join(_nstuck), 34) +
                                                ") — глянь до одобрения") if _nstuck else \
                            "герой-цифры сверены с вебом; остался числовой нюанс — глянь в Отложенных"
                        out("⚠️ 2FA веб-сверка: остался числовой нюанс/формулировка — публикую, глянь в "
                            "«Отложенных»:\n" + str(sv2) + "\n")
                else:
                ...
                panel["🔎 2FA scope"] = ("цифры сверены с вебом; есть непрослеженное — глянь ❓ ниже"
                                        if _unver else "✓ цифры сверены с реальными источниками (веб)")
            # НЕПРОСЛЕЖЕННОЕ (❓) — отдельной строкой владельцу. Публикацию не блокирует (см. has_issues),
            # но и молчать нельзя: 26.08 приписанный ФРС тезис уехал в отложку, пока панель писала «✓».
            if _unver:
                panel["❓ проверь вручную"] = _clip("; ".join(_unver), 46)
                out("❓ 2FA не смог проследить это по брифу (пост НЕ правил — вырезать валидное нельзя):\n   "
                    + "\n   ".join(_unver) +
                    "\n   Сверь по ПЕРВОИСТОЧНИКУ. Не подтвердилось — убери строку или сними атрибуцию "
                    "(«по данным регулятора» → редакционный вывод), пока пост в Отложенных.\n")
        ...
        # ГЕЙТ ВЫХОДА — КОДОМ (12.09). §7.4/§7.45 требуют в мете [[УЗЕЛ]] и [[ВЫХОД]] («что читатель
        # сделает или подумает завтра иначе»), но требование жило просьбой в промпте. Прогон 12.09:
        # писатель не выдал НИ ОДНОЙ строки меты, пайплайн опубликовал, владелец забраковал пост —
        # «пользы 0, не ясно ни что делать, ни как прочитанное использовать». Даём ОДИН прицельный
        # круг; не помогло — говорим об этом в панели прямо, а не молчим (пост всё равно обязан быть).
        try:
            _mb = creator_tools.scope_meta_defects(verify.latest_draft("scope") or post or "")
            if _mb:
                out("🚪 ГЕЙТ ВЫХОДА: " + "; ".join(_mb) + "\n   Даю один прицельный круг — назвать "
                    "узел и выход, и добрать инструмент в тело, если его там нет.\n")
                post = _threaded(scope_writer.fix_meta, "; ".join(_mb), fkey) or post
                _mb2 = creator_tools.scope_meta_defects(verify.latest_draft("scope") or post or "")
                if _mb2:
                    panel["🚪 выход"] = "⚠ " + _clip("; ".join(_mb2), 44)
                    out("⚠️ ВЫХОД так и НЕ НАЗВАН: " + "; ".join(_mb2) + "\nЭто значит, что пост даёт "
                        "понимание, а не защиту — решай, публиковать ли (§7.45).\n")
                else:
                    panel["🚪 выход"] = "назван (узел + выход в мете)"
        except Exception:
            logging.exception("гейт выхода scope упал — пропускаю (сбой не блокирует публикацию)")
        # ЖИВАЯ РЕЧЬ (14.09). Владелец: «проверка должна быть ещё на то — говорят ли так люди; оно такую
        # хуйню выкручивает, что точно видно, что это ИИ» («у станка одна педаль», «раздули на долг»).
        # Кодом это не меряется: замер по 365 постам автора дал шум (23 «новых» слова, из них плохих два).
        # Поэтому — отдельный ЧИТАТЕЛЬ, который только показывает фразы (замер на двух постах 14.09: 3 из 4
        # правок владельца пойманы), а правит их писатель одним кругом. Идёт ДО перепроверки финала и
        # длины ниже, чтобы панель описывала уже исправленный текст.
        try:
            _flags = scope_writer.speech_flags(verify.latest_draft("scope") or post or "", fkey)
            if _flags:
                out("🗣 ЖИВАЯ РЕЧЬ: читатель пометил фразы, которые так не говорят —")
                for _f in _flags[:6]:
                    out("   • " + _f[:220])
                post, _saved = _threaded(scope_writer.fix_speech, _flags, fkey)
                panel["🗣 речь"] = (f"{len(_flags)} фраз(ы) переписаны: " if _saved
                                   else f"⚠ {len(_flags)} фраз(ы) НЕ переписаны (правка не сохранилась): ") \
                    + _clip(_flags[0], 34)
                out("")
            else:
                panel["🗣 речь"] = "читатель не нашёл неживых фраз"
        except Exception:
            logging.exception("проверка живой речи упала — пропускаю (не блокирую пост)")
        # РЕДАКТОР КАНАЛА (16.09). Анти-повтор судит ТЕМУ до письма, а пост успевает приехать в вывод,
        # который канал делал сто раз, и прицепить абзац не по теме. Правки владельца 16.09: «"приток в
        # ETF — плохая опора" — миллион раз повторялось на канале»; «"про сам биткоин — тишина" —
        # притянуто за уши». Между «тема не повтор» и «текст готов» не стояло ничего: судьи текста сняты
        # 31.07, линтер с тех пор меряет форму, не смысл. Механизм тот же, что у речи: редактор ПОКАЗЫВАЕТ,
        # писатель правит сам одним кругом — судью-переписывателя не воскрешаем, он стачивал голос.
        try:
            _echo = scope_writer.echo_flags(verify.latest_draft("scope") or post or "", fkey)
            if _echo:
                out("🔂 РЕДАКТОР КАНАЛА: это мы уже говорили / абзац не про тему —")
                for _e in _echo[:5]:
                    out("   • " + _e[:220])
                post, _esaved = _threaded(scope_writer.fix_echo, _echo, fkey)
                panel["🔂 уже говорили"] = (f"{len(_echo)} шт исправлено: " if _esaved
                                           else f"⚠ {len(_echo)} шт НЕ исправлено (правка не легла): ") \
                    + _clip(_echo[0], 34)
                out("")
            else:
                panel["🔂 уже говорили"] = "редактор не нашёл повтора вывода"
        except Exception:
            logging.exception("редактор канала упал — пропускаю (не блокирую пост)")
        # ФОРМА ФИНАЛА И СТРУКТУРА — ВЛАДЕЛЬЦУ, А НЕ ТОЛЬКО АВТОРУ (07.08). Линтер возвращает эти
        # претензии автору при save_draft — но автор их может ЗАБОЛТАТЬ, и 07.08 именно это и произошло:
        # на предупреждение о финале модель ответила «линтер, судя по всему, срабатывает на паттерн из
        # предыдущей версии и кешируется. Финал чистый» — и выдала финал-вопрос, который владелец потом
        # переписал руками. Судью-переписывателя не воскрешаем (снят 31.07, он стачивал голос): просто
        # ПЕРЕПРОВЕРЯЕМ тем же кодом уже готовый драфт и, если дефект на месте, честно говорим об этом в
        # панели. Ноль вызовов модели, ноль правок текста — только «автор проигнорировал, смотри сюда».
        try:
            ...
            # + ЖИРНЫЙ (10.09): ровно тот класс дефектов, что владелец правил РУКАМИ в вышедшем
            # посте. Линтер видит их и на сохранении драфта, но после судей текст переписывается —
            # смотрим ФИНАЛЬНУЮ версию, иначе разметка чинится там, где её потом снесут.
            _sd = [w for w in creator_tools._lint(_final_text, "scope")[1]
                   if "ПРОСТЫНЯ" in w or "СЛЕПЛЕНО" in w or "союз «а» выпал" in w
                   or "акцент" in w or "жирн" in w]
            ...
            # ЧТО СДЕЛАЛ ПРЕДОХРАНИТЕЛЬ ДЛИНЫ (12.09). Ампутация уходила в logging.warning, то есть
            # никуда: 12.09 код удалил абзац с пейоффом читателю, владелец увидел в панели только
            # «📏 длина: 1314» и решил, что пост таким и написали. Теперь действие резчика стоит рядом.
            if scope_writer.LAST_LEN_ACTION:
                panel["✂️ резчик"] = scope_writer.LAST_LEN_ACTION
                if scope_writer.LAST_LEN_ACTION.startswith("⚠"):
                    out("✂️ " + scope_writer.LAST_LEN_ACTION + "\n")
            if _fd or _sd:
                ...
                out("✍️ ЛИНТЕР ПОВТОРНО (по тому, что реально уйдёт в канал): автор оставил замечания "
                    "без правки —")
                for _w in (_fd + _sd)[:3]:
                    out("   • " + _w.replace("scope: ", ""))
                out("   Пост не блокирую и не переписываю чужой рукой — поправь в «Отложенных».\n")
        except Exception:
            logging.exception("перепроверка формы финала/структуры упала — пропускаю (не блокирую пост)")
    ...
    if not scope and (post or "").strip():
        ...
        try:
            ...
            cover_path = ""
        # ОТЧЁТ ПО ПУЛУ — В ПАНЕЛЬ (31.08). Раньше при отказе владелец видел только «кадра нет» и не мог
        # понять, почему: две статьи-первоисточника молча отдали 429/403, а в пул попал второй эшелон
        # с ИИ-шапками. Теперь видно поимённо, кто что дал.
        pool = scope_writer.LAST_POOL_NOTE
        pick = scope_writer.LAST_COVER_NOTE
        if cover_path:
            panel["🖼 обложка"] = f"кадр из открытых источников ({pick})" if pick else "кадр из открытых источников"
            out(f"🖼 Обложка выбрана (по смыслу подходит посту): {cover_path}")
        else:
            # ПОСТ БЕЗ КАРТИНКИ — ПРОВАЛ, А НЕ ШТАТНЫЙ ИСХОД (владелец 07.09: «картинка всегда, без
            # исключений»). До 07.09 панель писала об этом ровным тоном «уйдёт ТЕКСТОМ», и четыре
            # скоупа подряд ушли голыми, не вызвав тревоги. Теперь это алерт с причиной.
            panel["🖼 обложка"] = f"⚠️ НЕТ — пост ушёл голым текстом ({pick})" if pick \
                else "⚠️ НЕТ — пост ушёл голым текстом"
            out("🚨 ОБЛОЖКИ НЕТ — АВАРИЯ, а не норма. Обложка ставится ВСЕГДА, если скачался хотя бы\n"
                "   один кадр (владелец 07.09: «всегда, без исключений»). Пусто здесь означает ровно\n"
                "   одно: НИ ОДИН источник не отдал картинку. Смотри пул ниже — кто именно не ответил.")
        if pool:
    ...
    # Квитанция пустая = в отложку НЕ легло (упала сеть/нет канала/нет драфта — причина напечатана выше).
    # До 11.09.2026 дальше шло как при успехе: панель писала «✅ в отложке», пост уходил в журнал вышедших
    # (с репликой модели — квитанции нет), тема флагмана помечалась [вышло]. Threads-ветка потом считала бы
    # такой пост вышедшим, а пикер полгода не давал бы тему, которая в канал так и не попала.
    if receipt:
        panel["публикация"] = "✅ в отложке канала (проверь и одобри)"
    else:
        panel["публикация"] = "❌ НЕ поставлено в отложку — причина выше"
        out("⛔ В отложку не легло — в журнал вышедших не пишу и тему не помечаю: пост в канал не вышел.")
    ...
    if receipt and theme and not scope:

...
def run_cycle(scope: bool = False, skip_scout: bool = False, draft_only: bool = False,
    ...
    if not scope and (post or "").strip():
        ...
        if cover_path:
            ...
            out("⚠️ ОБЛОЖКИ НЕТ — это провал прогона, а не норма: скоуп обязан находить кадр в открытых\n"
                "   источниках (инструкция memory/scope_cover_manual.md). Рисовать ему запрещено — это\n"
                "   только флагман. Смотри пул ниже: если кадры были, ошибся судья; если пул пуст —\n"
                "   не отдали источники.")
        if pool:

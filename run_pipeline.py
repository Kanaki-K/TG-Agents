...
def _run_creator(command: str = "post", avoid: str = "", hint: str = "", bank: list | None = None,
    ...
    if evergreen:  # ВЕЧНАЯ ТЕМА: осознанно НЕ новость — образовательный флагман из банка вечных тем.
        ...
        guard += ("Возьми ОДНУ тему из банка:\n"
                  + "\n".join(f"- {t}" for t in (bank or [])) + "\n"
                  "ОКНО ПОВТОРА — 6 МЕСЯЦЕВ. Сверься со списком постов канала в контексте: если ТЕМА или её "
                  "ТЕЗИС выходили за последние ~6 месяцев — НЕ бери, возьми другую. Канал даёт 1-2 флагмана "
                  "в неделю, повтор в пределах полугода читателю очевиден. Смотри не только точное "
                  "совпадение, но и СОСЕДНИЙ домен: «смерть альтов» = «90% умирают» = «выживают лидеры» = "
                  "одно и то же для читателя.\n"
                  "НЕ своди пост к морали «делай DCA/усреднение» — эту мысль канал разбирал много раз. Если "
                  "тема не ПРО усреднение напрямую, вывод про DCA НЕ лепи.\n"
                  "ГЛАВНОЕ — ПОДАЧА: раскрой тему с ножевым, контрарным поворотом (эталон «доллар»: факт → "
                  "неожиданный разворот). Хук-клише («уже поздно», «все говорят», «представьте») — ЗАПРЕЩЁН. "
                  "Звучи как эталоны подачи. НАПИШИ ГОТОВЫЙ ПОСТ — текст, не мета-рассуждение. Сохрани save_draft.\n\n")
        user = guard + user

    ...
    if scope and post:
        ...
        try:
            ...
            if verify.has_issues(sv):
                ...
                # выдуманный УЧАСТНИК/атрибуция — НЕ «мелкий нюанс»: его нельзя опубликовать даже пометкой
                # (баг 29.07: пост ушёл с невериф. «BlackRock — клиент DTA», ре-гейт счёл нюансом). Числовой
                # конфликт fix_facts чинит подстановкой верного значения; выдуманную сущность — только СНОСИТ.
                # Ещё один прицельный проход на снос, затем перепроверка.
                if verify.has_redline(sv2):
                    out("⛔ 2FA: остался НЕПРОВЕРЕННЫЙ участник/источник (красная линия — выдумку публиковать "
                        "нельзя) — сношу и перепроверяю:")
                    post = _threaded(scope_writer.fix_facts, sv2, fkey) or post
                    sv2 = verify.verify_post(verify.latest_draft(), verify.latest_brief(), api_key=fkey,
                                             scope=True, web=True)
                if verify.has_redline(sv2):
                    # Снос не удался (крайне редко) — НЕ публикуем выдумку тихо: громкий флаг владельцу.
                    panel["🔎 2FA scope"] = "⛔ невериф. участник не снят авто — убери ВРУЧНУЮ до одобрения"
                    out("⛔ ВНИМАНИЕ: непроверённый участник/источник авто-снять не удалось. НЕ одобряй пост, "
                        "пока не уберёшь его вручную в «Отложенных»:\n" + str(sv2) + "\n")
        ...
        # ✍️ ЗАВЕРШЁННОСТЬ МЫСЛЕЙ — enforcement завершённых предложений (правило 27.07: обрывки «тот же
        # триггер - не двинулся» = мучительная ручная правка тела; редактура-промптом их пропускала).
        # Фокусный судья (как финал §4.5) достраивает ТОЛЬКО обрывки, рубленую краткость не трогает;
        # код-envelope бережёт от порчи тела. Идёт ПЕРЕД финалом — тот дожмёт кикер последним.
        try:
            out("✍️ [Мысли] Достраиваю обрывки до завершённых предложений (краткость не трогаю)...")
            _fr_before = (post or "").split("[[SPLIT]]")[0].rstrip()
            post = _threaded(scope_writer.fix_fragments, fkey) or post
            _fr_changed = (post or "").split("[[SPLIT]]")[0].rstrip() != _fr_before
            panel["✍️ мысли"] = "обрывки достроены" if _fr_changed else "✓ мысли завершены"
        except Exception:
            logging.exception("Проход завершённости мыслей scope упал — публикую как есть")

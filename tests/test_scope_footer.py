...
def test_pipe_metaphor_banned():
    ...
    assert any("БАН-ОБРАЗ" in w and "труб" in w for w in warns)


def test_invented_images_from_measure_are_banned():
    """12.09: словарь расширен ЗАМЕРОМ (339 постов), а не на глаз — у каждого слова ноль вхождений
    в канале при живой альтернативе рядом («в минусе» 15, «холдер» 14, «средняя цена» 20)."""
    for phrase, hint in (("Уровень работает крышей над рынком", "крыш"),
                         ("Почти 38% холдингов сейчас под водой", "под водой"),
                         ("Ставки - не вотчина ФРС", "вотчин"),
                         ("В июле раскатили пилот на миллиард", "раскат"),
                         ("Это конкретные руки, которые побегут продавать", "руки")):
        warns = ct._lint(_post([phrase]), "scope")[1]
        assert any("БАН-ОБРАЗ" in w and hint in w for w in warns), phrase


def test_entry_is_not_a_price_level():
    """«Вход» — одна покупка одного человека; уровень толпы = безубыток / средняя цена покупки."""
    warns = ct._lint(_post(["Они побегут на первом же возврате к входу"]), "scope")[1]
    assert any("это ОДНА покупка" in w for w in warns)
    # сцепка «средняя цена входа» — живая форма канала (5 вхождений), её НЕ трогаем
    ok = ct._lint(_post(["Средняя цена входа всех фондов - около 83 000$"]), "scope")[1]
    assert not any("ОДНА покупка" in w for w in ok)

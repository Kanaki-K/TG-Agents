...
# ── ПИСАТЕЛЮ ВЕЛЕЛИ ИСКАТЬ НЕ ТАМ (07.09) ───────────────────────────────────────────────────────
# Инструкция [[MEDIA_SRC]] с июля требовала СНАЧАЛА официальный сайт компании, а статьи — «добери
# потом». Замер 23 обложек говорит обратное: почти все они — шапки новостных статей и пресс-фото,
# подобранные редактором под конкретное событие. Официальный сайт даёт «фирму вообще» — ту самую
# башню и вход в отделение, которые владелец забраковал 07.09.

def test_writer_is_asked_for_articles_not_company_pages():
    ask = sw.TASK if hasattr(sw, "TASK") else ""
    src = next((v for k, v in vars(sw).items()
                if isinstance(v, str) and "[[MEDIA_SRC]]" in v and "web_search" in v), ask)
    assert "СТАТЬИ ИЗДАНИЙ" in src, "просим статьи изданий, а не страницы компаний"
    assert "Официальный сайт компании в этот список НЕ НУЖЕН" in src
    assert "сперва тир-1" in src, "тир-1 первыми: их художку и берут 23 принятые обложки"
    assert "СНАЧАЛА ПЕРВОИСТОЧНИК" not in src, "старый порядок вернулся — обложки поедут в «фирму вообще»"

...
def test_finale_defuse_outside_finale_para_untouched(monkeypatch):
    """РЕГРЕССИЯ Strategy 31.07 — ДУБЛЬ АБЗАЦА В КАНАЛЕ.

    Финал УЖЕ расшит (оговорка и кикер — разные абзацы), но судья смотрит весь пост и возвращает
    «РАСШИТЬ», где оговорка взята из ПРЕДЫДУЩЕГО абзаца. Старый код писал её поверх финального абзаца и
    вставлял кикер ниже → абзац дублировался дословно («Тезис на BTC не сломался» дважды подряд) и ушёл
    в отложку. Теперь обе части обязаны происходить ИЗ последнего абзаца — иначе не трогаем."""
    saved: dict = {}
    draft = ("**⚡️ Заголовок поста**\n\n"
             "Первый абзац тела с фактом и цифрой\n\n"
             "Тезис на BTC не сломался\n\n"
             'Но "покупателя последней инстанции", который никогда не продаёт, больше нет\n\n'
             f"{FOOTER}")
    _setup(monkeypatch, draft,
           'РАСШИТЬ: Тезис на BTC не сломался @@@ Но "покупателя последней инстанции", '
           "который никогда не продаёт, больше нет", saved)
    out = sw.fix_finale()
    assert out == draft                                   # пост не тронут
    assert saved == {}                                    # драфт не пересохранён
    assert out.count("Тезис на BTC не сломался") == 1     # ГЛАВНОЕ: дубля нет

...
# --- КРАСНАЯ ЛИНИЯ НЕ ЛОВИТСЯ НА ПОХВАЛУ (баг 28.08) -------------------------------------------
# Вердикт по посту про Solana закончился строкой «Единственная правка: ⚠️ МЕХАНИКА: …подняли… Остальные
# утверждения поста — верифицированы.». Маркер «верифиц» стоял в списке БЕЗ отрицания и поймал ровно
# слово «верифицированы», то есть ПОХВАЛУ. Пайплайн решил, что осталась выдумка, прогнал снос-проход и
# доложил владельцу «был невериф. участник — снёс». Участника не было: правка сделала «подняли» →
# «решили поднять», то есть работу класса МЕХАНИКА, чья ветка из-за перехвата не запустилась.

_PRAISE_MECHANICS_LINE = ("Единственная правка: ⚠️ МЕХАНИКА: «годовой темп сокращения эмиссии подняли» — "
                          "голосование дало мандат разработчикам, параметр ещё не изменён. Заменить на "
                          "«проголосовали поднять». Остальные утверждения поста — верифицированы.")


def test_positive_verification_is_not_a_redline():
    assert verify.has_redline(_PRAISE_MECHANICS_LINE) is False


def test_positive_verification_still_routes_to_mechanics():
    """Ветку МЕХАНИКА красная линия больше не перехватывает — иначе класс лечится не своим проходом."""
    assert verify.has_mechanics(_PRAISE_MECHANICS_LINE) is True


def test_negated_verification_is_still_a_redline():
    """Отрицание осталось красной линией — сужали маркер, а не отменяли правило."""
    v = "⚠️ АТРИБУЦИЯ: «по данным Schwab» — источник НЕ верифицирован ни брифом, ни вебом"
    assert verify.has_redline(v) is True
    v2 = "⚠️ «BlackRock — клиент DTA» — невериф. участник, убрать"
    assert verify.has_redline(v2) is True

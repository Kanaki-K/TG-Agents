...
# ── ❓, КОТОРЫЙ 2FA САМ СНЯЛ, НЕ ИДЁТ ВЛАДЕЛЬЦУ (12.09.2026) ─────────────────────────────────────
# Прогон 12.09: в панель под шапкой «проверь вручную по первоисточнику» уехала строка «❓ "Токенизация
# обещала убрать посредника" — это авторская обобщающая фраза поста… НЕ ПОДЛЕЖИТ ПРОВЕРКЕ как
# конкретная атрибуция». Проверять там нечего — вердикт закрыл вопрос сам, а владелец получил задание
# на пустом месте. Он на это и ругался: «вопросов больше чем ответов».

def test_self_dismissed_question_is_dropped():
    v = ("ИТОГ: 8✅ / 0⚠️ / 1❓\n"
         "   9. ❓ «Токенизация обещала убрать посредника» — это авторская обобщающая фраза поста "
         "(не атрибуция конкретному лицу). Не подлежит проверке как конкретная атрибуция.")
    assert verify.unverified_notes(v) == []


def test_real_question_still_reaches_the_owner():
    """Случай 26.08, ради которого функция и написана: тезис приписан ФРС, в брифе не прослеживается —
    такое решает человек, и оно обязано доехать до панели."""
    v = ("❓ «в кредитах растёт доля рискованных заёмщиков» приписано ФРС — в брифе не прослеживается, "
         "проверь вручную по самой бумаге")
    assert len(verify.unverified_notes(v)) == 1


def test_author_conclusion_wording_variants_are_dropped():
    for s in ("❓ «рынок устал» — собственный вывод автора, проверять нечего",
              "❓ фраза про посредника — риторический тезис поста, не требует проверки",
              "❓ «это меняет расклад» — редакционный вывод, не атрибуция"):
        assert verify.unverified_notes(s) == [], s
